...
const explorer = cosmiconfigSync(
    'auto-logging',
    {
        searchPlaces: ['logging.yml', 'logging.yaml'],
...
function _messageFormat() {
    let template = conf.get(conf.keys.Template,
...
function _processFormatOptions() {
    ...
    let formatKey = conf.keys.Formats;
...
/**
 * Saves the global logger object in the logging namespace. For internal use only!
...
function _createConfiguredLogger() {
    const workspace = path.dirname(explorer.search().filepath);
    let winstonOptions = {};
    let logFormats = conf.get(conf.keys.FormatsRoot);
    ...
    let logTargets = conf.get(conf.keys.Targets);
    if (typeof logTargets !== 'object') {
        throw new Error('The "logging-targets" attribute must be an object');
    }
    ...
    for (let target of Object.keys(logTargets)) {
        ...
        let enabled = conf.get(`logging-targets-${target}-enabled`);
...
function getLogger(name) {
    // wrap the global logger if exists
    if (global.logging) {
        return _wrapWithLoggerName(global.logging, name);
    }

    // The global logger must be created
